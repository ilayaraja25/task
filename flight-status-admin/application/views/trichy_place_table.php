...
<body>
    ...
    <div class="container box">
        <h3 align="center">Master Airports Table</h3></br>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function() {
        function fetch_data() {
            $.ajax({
                url: "<?php echo base_url('Trichy_airport/get_trichymaster_place_table') ?>",
                method: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    for (var count = 0; count < a.length; count++) {
                        html += '<tr>';
                        html += '<td id="id_t" class="table_data" data-row_id="' + a[count].place_id + '" data-column_name="id" >' + a[count].place_id + '</td>';

                        html += '<td id="place_t" class="table_data" data-row_id="' + a[count].place_id + '" data-column_name="place" contenteditable>' + a[count].place + '</td>';
                        // html += '<td id="terminal_t" class="table_data" data-row_id="' + a[count].place_id + '" data-column_name="terminal" contenteditable>' + a[count].terminal + '</td>';
                        // html += '<td id="state_t" class="table_data" data-row_id="' + a[count].place_id + '" data-column_name="state" >' + a[count].state + '</td>';
                        html += '<td id="time_stamp_t" class="table_data" data-row_id="' + a[count].place_id + '" data-column_name="time_stamp" >' + a[count].time_stamp + '</td>';
        ...
        $(document).on('click', '#btn_add', function() {
            ...
            */
            $.ajax({
                url: "<?php echo base_url('Trichy_airport/insert_master_place_table'); ?>",
                method: "POST",
                data: {
                    place: origin,
                    terminal: terminal
        ...
        $(document).on('blur', '.table_data', function() {
            ...
            if (confirm("Are you sure you want to update this?")) {
                $.ajax({
                    url: "<?php echo base_url('Trichy_airport/update_master_place_table'); ?>",
                    method: "POST",
                    //dataType:"JSON",
                    data: {
                        id: id,
        ...
        $(document).on('click', '.btn_delete', function() {
            ...
            if (confirm("Are you sure you want to delete this?")) {
                $.ajax({
                    url: "<?php echo base_url('Trichy_airport/delete_master_place_Table'); ?>",
                    method: "POST",
                    data: {
                        id: id
                    },
