...
<script type="text/javascript" language="javascript">
    $(document).ready(function() {
        function fetch_data() {
            $.ajax({
                url: "<?php echo base_url('Airport_Controller/get_trichymaster_place_table') ?>",
                method: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    for (var count = 0; count < a.length; count++) {
						...
						html += '<td id="airport_name_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="airport_name" contenteditable>' + a[count].airport_name + '</td>';
                        html += '<td id="airport_code_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="airport_code" contenteditable>' + a[count].airport_code + '</td>';
                        html += '<td id="city_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="city" contenteditable>' + a[count].city + '</td>';
                        html += '<td id="country_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="country" contenteditable>' + a[count].country + '</td>';
                    ...
                    html1 += '<td id="state" value="State"></td>';

                    html1 += '<td id="time_stamp" value="time_stamp"></td>';
        ...
        $(document).on('click', '.btn_delete', function() {
            ...
            if (confirm("Are you sure you want to delete this?")) {
                $.ajax({
                    url: "<?php echo base_url('Airport_Controller/delete_master_place_Table'); ?>",
                    method: "POST",
                    data: {
                        id: id
                    },
