...
<body>
    ...
    <div class="container box">
        <h3 align="center">Master Places Table</h3></br>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function() {
        function fetch_data() {
            $.ajax({
                url: "<?php echo base_url('Trichy_airport/get_trichymaster_flight_table') ?>",
                method: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    for (var count = 0; count < a.length; count++) {
                        html += '<tr>';
                        html += '<td id="id_t" class="table_data" data-row_id="' + a[count].flight_id + '" data-column_name="id" >' + a[count].flight_id + '</td>';

                        html += '<td id="flight_t" class="table_data" data-row_id="' + a[count].flight_id + '" data-column_name="flight" contenteditable>' + a[count].flight + '</td>';
                        html += '<td id="terminal_t" class="table_data" data-row_id="' + a[count].flight_id + '" data-column_name="terminal" contenteditable>' + a[count].terminal + '</td>';
                        html += '<td id="state_t" class="table_data" data-row_id="' + a[count].flight_id + '" data-column_name="state" >' + a[count].state + '</td>';
                        html += '<td id="time_stamp_t" class="table_data" data-row_id="' + a[count].flight_id + '" data-column_name="time_stamp" >' + a[count].time_stamp + '</td>';
                        html += '<td><button type="button" name="delete_btn" id="' + a[count].flight_id + '" class="btn btn-xs btn-danger btn_delete"><span class="glyphicon glyphicon-remove"></span></button></td></tr>';

                    }

                    html1 += '<tr>';

                    html1 += '<td id="place" contenteditable placeholder="Enter the Flight"></td>';
        ...
        $(document).on('click', '#btn_add', function() {
            ...
            */
            $.ajax({
                url: "<?php echo base_url('Chn_airport/insert_master_place_table'); ?>",
                method: "POST",
                data: {
                    place: origin,
                    terminal: terminal
        ...
        $(document).on('blur', '.table_data', function() {
            ...
            if (confirm("Are you sure you want to update this?")) {
                $.ajax({
                    url: "<?php echo base_url('Chn_airport/update_master_place_table'); ?>",
                    method: "POST",
                    //dataType:"JSON",
                    data: {
                        id: id,
