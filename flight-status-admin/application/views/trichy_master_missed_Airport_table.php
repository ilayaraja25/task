...
<body>
    ...
    <div class="container box">
        <h3 align="center">Master Missed Airport Table</h3></br>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function() {
        function fetch_data() {
            $.ajax({
                url: "<?php echo base_url('Trichy_airport/get_trichymaster_missed_Airport_table') ?>",
                method: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    for (var count = 0; count < a.length; count++) {
                        html += '<tr>';
                        html += '<td id="id_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="si_no" >' + a[count].si_no + '</td>';

                        html += '<td id="place_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="place" contenteditable>' + a[count].place + '</td>';
                        // html += '<td id="status_t" class="table_data" data-row_id="' + a[count].si_no + '" data-column_name="status" contenteditable>' + a[count].status + '</td>';
        ...
        $(document).on('click', '.btn_delete', function() {
            ...
            if (confirm("Are you sure you want to delete this?")) {
                $.ajax({
                    url: "<?php echo base_url('Trichy_airport/delete_master_missed_airport_Table'); ?>",
                    method: "POST",
                    data: {
                        id: id
